...
export function expandDasriFormFromDb(form: DasriForm): GqlDasriForm {
  return {
    id: form.id,
    ...
    emission: nullIfNoValues<Emission>({
    ...
    transport: nullIfNoValues<Transport>({
      wasteDetails: nullIfNoValues<DasriWasteDetails>({
      ...
      wasteAcceptation: nullIfNoValues<WasteAcceptation>({
    ...
    reception: nullIfNoValues<Reception>({
      wasteDetails: nullIfNoValues<DasriWasteDetails>({
...
function flattenReceptiontInput(input: { reception?: DasriReceptionInput }) {
  return {
    recipientWasteQuantity: chain(input.reception, e =>
      chain(e.wasteDetails, w => w.quantity)
    ),
    recipientWasteVolume: chain(input.reception, e =>
      chain(e.wasteDetails, w => w.volume)
    ),
    processingOperation: chain(input.reception, e => e.processingOperation),
    receivedAt: chain(input.reception, e =>
      e.receivedAt ? new Date(e.receivedAt) : null
    ),
    processedAt: chain(input.reception, e =>
      e.processedAt ? new Date(e.processedAt) : null
    )
  };
}
